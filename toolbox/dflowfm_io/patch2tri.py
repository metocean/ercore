...
# Quads where 2--4 is shortest diagonal: tri's are (1-based) 1-2-4 and 2-3-4
   ...
   # EDIT S.WEPPE there may be a mistake below
   #  originally it was :
   #  ntri1 =  sum(ismaindiag.nonzero()) which sums the indices where ismaindiag~=0
   #  while it should only sum the number of times ismaindiag~=0 ?
   # ntri1 =  sum(ismaindiag.nonzero())
   ntri1 =  numpy.count_nonzero(ismaindiag)
